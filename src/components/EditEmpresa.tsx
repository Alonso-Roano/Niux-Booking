...
import negocioDefault from "/images/negocio-default.webp";
...
export const EditEmpresa: React.FC = () => {
  ...
  const handleGuardarCambios = async () => {
    ...
    if (empresaNombre || selectedCategoria !== "Selecciona una categoría") {
      try {
        ...
        const response = await niuxApi.put(
          `/Empresa/ActualizarEmpresa/${idEmpresa}`,
          payload
        );
        ...
        Utils.showToast({
          icon: "error",
          title: "Error al actualizar los datos de la empresa.",
        });
      }
    }
    ...
    if (empresaImagen) {
      try {
        ...
        const response = await niuxApi.put(
          "/Empresa/ActualizarImagenEmpresa",
          formData
        );
  ...
  const fetchEmpresa = async (idEmpresa: number) => {
    try {
      ...
      setSelectedCategoria(
        response.data.data.idCategoria || "Selecciona una categoría"
      );
  ...
  const handleImageDelete = async () => {
    try {
      const response = await niuxApi.delete(
        `/Empresa/EliminarImagenEmpresa/${idEmpresa}`
      );
